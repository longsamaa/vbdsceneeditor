...
import { applyShadowLitMaterial, transformModel } from '../model/objModel.ts';
...
export type DataTileInfoLayerForTubeLayer = {
    sceneTile: THREE.Scene;
    shadowLitMaterials: ShadowLitMaterial[];
    shadowMaterials: ShadowPair[];
}

export class TubeLayer implements Custom3DTileRenderLayer, ShadowCasterLayer, ReflectionCasterLayer {
    id: string;
    ...
    private tileCache: Map<string, DataTileInfoLayerForTubeLayer> = new Map<string, DataTileInfoLayerForTubeLayer>();
    ...
    constructor(opts: TubeLayerOpts & { onPick?: (info: PickHit) => void } & { onPickfail?: () => void }) {
        this.id = opts.id;
    ...
    private getTileData(key: string): DataTileInfoLayerForTubeLayer {
            ...
            if (!tileDataInfo) {
                ...
                for (const feature of layer.features) {
                    ...
                    for (const ring of geom) {
                        ...
                        const points = ring.map(p => new THREE.Vector3(p.x, p.y, elevation * scaleUnit));
                        const path = new THREE.CurvePath<THREE.Vector3>();
                        for (let i = 0; i < points.length - 1; i++) {
                            path.add(new THREE.LineCurve3(points[i], points[i + 1]));
                        }
                        const tubeGeom = new THREE.TubeGeometry(path, points.length * 4, TUBE_RADIUS * scaleUnit, TUBE_SEGMENTS, false);
                        tubeGeometries.push(tubeGeom);
                    }
                }
                if (tubeGeometries.length > 0) {
                    const merged = mergeGeometries(tubeGeometries);
                    if (merged) {
                        merged.computeVertexNormals();
                        const material = new THREE.MeshStandardMaterial({ color: this.color });
                        const mesh = new THREE.Mesh(merged, material);
                        const shadowLitMats = applyShadowLitMaterial(mesh);
                ...
                // Fine grid
                const gridFine = new THREE.GridHelper(8192, 32, 0x444444, 0x444444);
                gridFine.rotation.x = Math.PI / 2;
                gridFine.position.set(4096, 4096, 0.2);
                gridFine.renderOrder = 1;
                scene.add(gridFine);
                // Coarse grid
                const gridCoarse = new THREE.GridHelper(8192, 8, 0x888888, 0x888888);
                gridCoarse.rotation.x = Math.PI / 2;
                gridCoarse.position.set(4096, 4096, 0.3);
                gridCoarse.renderOrder = 1;
    ...
    renderReflection(renderer: THREE.WebGLRenderer, reflectionMatrix: THREE.Matrix4, worldSize: number): void {
        ...
        const tr = this.map.transform;
        const tilesWithShadow: DataTileInfoLayerForTubeLayer[] = [];
        for (const tile of this._visibleTiles) {
    ...
    mainPass(tr: any, visibleTiles: OverscaledTileID[]) {
        ...
        for (const tile of visibleTiles) {
            ...
            //tileInfo.sceneTile.traverse(obj => { obj.frustumCulled = false; });
    ...
    private updateShadowLitMaterials(tileInfo: DataTileInfoLayerForTubeLayer, tileKey: string) {
